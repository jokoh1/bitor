...
export async function fetchDigitalOceanData(provider: Provider): Promise<DigitalOceanData> {
    ...
    try {
        ...
        const [regionsResponse, projectsResponse, sizesResponse] = await Promise.all([
            ...
            provider.settings.region ? 
                fetch(`${baseUrl}/sizes?providerId=${provider.id}&region=${provider.settings.region}`, { headers }) :
        ...
        if (!projectsResponse.ok) {
            throw new Error(`Error fetching projects: ${await projectsResponse.text()}`);
        }
