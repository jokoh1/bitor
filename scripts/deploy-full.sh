...
# Full Stack Deployment Workflow Script
# Usage: ./scripts/deploy-full.sh [branch-name]
...
if [ -n "$1" ]; then
    BRANCH_NAME="feature/full-$1"
else
    BRANCH_NAME="feature/full-$(date +%Y%m%d-%H%M%S)"
fi

print_status "Starting full stack deployment workflow..."
print_status "Branch name: $BRANCH_NAME"
print_warning "This will deploy both frontend and backend changes"
...
print_status "Make your changes to both frontend/ and backend/ directories"
...
echo "1. Make your changes to frontend/ and/or backend/"
echo "2. git add ."
...
echo "6. Merge PR → Both Cloudflare Pages and Railway will deploy" 