...
# Backend Deployment Workflow Script
# Usage: ./scripts/deploy-backend.sh [branch-name]
...
if [ -n "$1" ]; then
    BRANCH_NAME="feature/backend-$1"
else
    BRANCH_NAME="feature/backend-$(date +%Y%m%d-%H%M%S)"
fi

print_status "Starting backend deployment workflow..."
...
print_status "Make your backend changes in the backend/ directory"
...
echo "1. Make your backend changes"
echo "2. git add ."
...
echo "6. Merge PR → Railway will auto-deploy" 